...
if (len(sys.argv) > 1):
    ...
    out_fc = arcpy.GetParameterAsText(1) #output fc with data projected onto desired cross sections
    xsln_fc = arcpy.GetParameterAsText(2) #used to list all mn_et_id values
    xs_id_field = arcpy.GetParameterAsText(3) #field that has xs numbers, usually et_id
    selected_xs = arcpy.GetParameterAsText(4) #user input list of xsecs to project to
    ...
    input_xs_num = #single value from xs_id_field
    output_xs_nums = #multiple values from xs_id_field
    out_gdb = r'D:\QuatStrat_Editing\Stacked_Testing\project_to_all_xsec.gdb\' #output fc with data projected onto desired cross sections
...
#%% 7 make a list of mn_et_id values based on xsln file. Integer data type.
printit("Listing cross section numbers.")

#TODO: set up selected xs list based on user input parameter
selected_xs_list =
...
#create list of mn_et_id values based on selected xs list
mn_et_id_list = []
with arcpy.da.SearchCursor(xsln_fc, [xs_id_field, "mn_et_id") as cursor:
    for row in cursor:
        et_id = row[0]
        #skip cross section number if it's not in the selected list
        if et_id not in selected_xs_list:
            continue
        mn_et_id = int(row[1])
...
arcpy.management.CreateFeatureclass(output_dir, output_name, shape)
