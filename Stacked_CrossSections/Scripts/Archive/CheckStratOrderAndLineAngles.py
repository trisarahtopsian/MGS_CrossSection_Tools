...
in memory workspace and output files are saved in the same gdb as the input
stratlines.
'''
...
if (len(sys.argv) > 1):
    ...
    stratlines_original = r'D:\Cross_Section_Programming\QC_tool_testing\qc_tool_testing_reverse.gdb\all_strat_vertices' #input fc with data drawn on one cross section
    stratline_unit_field = 'unit'
    unitlist_txt = r'D:\Cross_Section_Programming\QC_tool_testing\unitlist.txt'
    poly_ref = r'D:\Cross_Section_Programming\QC_tool_testing\qc_tool_testing_reverse.gdb\poly_ref'
...
#%% Create empty above polys for each stratline
...
#%% Create geometry for "above_polys" and check for angle errors
...
#%% Clip below unit files using polygon feature classes
arcpy.env.overwriteOutput = True
