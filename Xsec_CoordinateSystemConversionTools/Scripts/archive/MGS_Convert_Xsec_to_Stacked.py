...
# MGS Convert Xsec to Stacked
# Created by Sarah Francis, Minnesota Geological Survey
# Created Date: September 2022, updated October 2022
'''
This script takes any feature class that was created in the "true X" or "true Y"
...
if (len(sys.argv) > 1):
    ...
    system = arcpy.GetParameterAsText(2)
    ...
    in_fc = r'D:\DakotaSandModel\TINmodel_110122\SandModeling_TIN.gdb\profiles_surftopo'
    in_fc_etid_field = "et_id"
    system = 'true X'
    out_fc = r'D:\DakotaSandModel\TINmodel_110122\SandModeling_TIN.gdb\profiles_surftopo_stacked'
    xsln_fc = r'D:\DakotaSandModel\TINmodel_110122\SandModeling_TIN.gdb\xsln'
...
if system == "true X":
    true_x = True
    printit("Input file is in true X system.")
if system == "true Y":
    true_y = True
        ...
        for line in xsln_cursor:
            ...
            with arcpy.da.SearchCursor(in_fc, ['SHAPE@X', 'SHAPE@Y', unique_id_field], where_clause) as point_cursor:
                for point in point_cursor:
                    ...
                    if true_x:
                        new_x = x
                        #remove VE from y coordinate and convert to meters
                        y_raw = y * 3.280839895 / in_vertical_exaggeration
                        new_y = (((y_raw * 0.3048) - (county_relief * mn_et_id_int)) * out_vertical_exaggeration) + 23100000
