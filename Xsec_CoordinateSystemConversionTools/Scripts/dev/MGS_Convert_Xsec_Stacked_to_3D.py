...
cross section systems and converts the feature class to 3D true coordinates. 
There is no vertical exaggeration since all coordinates are real-world.
'''
...
if (len(sys.argv) > 1):
    ...
    in_fc = r'D:\SteeleStoryMap\c53_dvd\c53_dvd\spatial\Profiles_from_surfaces.gdb\strat_poly_23_stacked'
    in_fc_etid_field = "et_id"
    out_fc = r'D:\SteeleStoryMap\c53_dvd\c53_dvd\spatial\Profiles_from_surfaces.gdb\strat_poly_23_3d_test8'
    xsln_fc = r'D:\SteeleStoryMap\c53_dvd\c53_dvd\spatial\Quaternary_Stratigraphy.gdb\cross_section_lines'
...
#Create output, allowing Z values
arcpy.management.CreateFeatureclass(filepath, filename, shape, '', '', "ENABLED", spatialref)
        ...
        for line in xsln_cursor:
            ...
            with arcpy.da.SearchCursor(in_fc, ['SHAPE@X', 'SHAPE@Y', unique_id_field], where_clause) as point_cursor:
                for point in point_cursor:
                    ...
                    new_x = new_x_raw
                    #new y coordinate equals y value of cross section line, defined above
                    new_y = first_y
                    ...
                    new_z = ((y - 23100000) /(in_vertical_exaggeration * 0.3048) + ((county_relief * mn_et_id_int) / 0.3048))
                    
                    #make point object from new x and y coordinates
                    new_point = arcpy.Point(new_x, new_y, new_z)
                ...
                for line in line_cursor:
                    ...
                    try:    
                        for vertex in line[0].getPart(0):
                            ...
                            #new x is the same
                            new_x = new_x_raw
                            #new y is same as cross section y value
                            new_y = first_y
                            ...
                            point = arcpy.Point(new_x, new_y, new_z)
...
if shape == "Polygon":
    ...
    with arcpy.da.SearchCursor(xsln_fc, xsln_cursor_fields) as xsln_cursor:
        for xsln_line in xsln_cursor:
            ...
            if first_y != last_y:
                printerror("Error: xsln {0} vertices do not have the same y coordinate. Points will not plot correctly.".format(etid))
            #make y list of slightly different values so arc doesnt freak out about vertical polygons
            #y_list = [first_y + 0.005, first_y, first_y - 0.005]
            #count = 0
            #while count < 25:
            ...
            #min_x = min(x_pointlist)
            where_clause = "{0}='{1}'".format(in_fc_etid_field, etid)
            #create list for storing geometry objects
            #geometry_list = []        
            #search through strat vertex points along current xsln
            with arcpy.da.SearchCursor(in_fc, ['SHAPE@', unique_id_field], where_clause) as poly_cursor:
                for poly in poly_cursor:
                    ...
                    for vertex in poly[0].getPart(0):
                        ...
                        new_x = x
                        #y coordinate is the same as cross section y
                        #new_y = first_y + (0.0001*count)
                        new_y = first_y
                        ...
                        point = arcpy.Point(new_x, new_y, new_z)
                        vertex_list.append(point) 
                    #set first and last point to be the same
                    #vertex_list[-1] = vertex_list[0]
                    
                    array = arcpy.Array(vertex_list)
                    new_geometry = arcpy.Polygon(array, spatialref, True)
                    #geometry_list.append(new_geometry)
                    #insert geometry object into new file
